"""V2 orchestrator: composite discovery + frozen V1 semantics.

Discovery/provider layer is V2; extraction, evidence, classification, and
route evaluation are imported unchanged from runtime.discovery (frozen).
No municipality-specific logic, no provider-specific branching here.
"""
...
from runtime.discovery.classification import (
    AccessClassifier, EvidenceLedger, ProvenanceGraph, RouteEvaluator,
)
...
def run_v2(protocol, input_data, providers, fetch_provider=None):
    """Run one municipality through V2 discovery + frozen V1 semantics.

    providers: CompositeDiscoveryProvider (site-direct first). Each planner
    query runs through it; the first SUCCESS with results wins, and every
    response is preserved in observability output.
    """
    if not isinstance(protocol, dict):
        protocol = protocol.load()
    municipality = input_data["municipality"]
    run_id = "%s-v2" % municipality.lower().replace(" ", "-")
