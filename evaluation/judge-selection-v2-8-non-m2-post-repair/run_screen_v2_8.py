#!/usr/bin/env python3
"""V2.8 one-shot screening runner. Per-row checkpointing; no reruns.
Execution order frozen in execution-order.json: IDs ascending, DeepSeek complete,
then MiMo Pro complete. Deterministic-resolved fixtures consume zero judge calls."""
import json, sys, time
from pathlib import Path

HERE = Path(__file__).parent
sys.path.insert(0, str(HERE))
import judge_core_v2_8 as J
...
def main():
    ...
    for wire_id, key in CANDIDATES:
        ...
        for f in fx_all:
            ...
            rows.append(row)
            (HERE / f'screening-results-{key}.json').write_text(
                json.dumps({'candidate': key, 'model': wire_id, 'rows': rows},
                           ensure_ascii=False, indent=1) + '\n', encoding='utf-8')
            print(json.dumps({'candidate': key, 'id': fid, 'verdict': verdict,
                              'correct': correct, 'basis': basis}))
