#!/usr/bin/env python3
"""Task-local remeasure runner for NAV-EXPLORE-MEASUREMENT-V3-REMEASURE-REPAIR-WAVE-1.

Stage A (default): score the frozen Wave-1 v2 replay predictions with the
FROZEN combined measurement engine, generate fresh semantic review packets,
run leakage + reviewability audits, and a determinism re-pass.
...
BASE.P3_RUNS = os.path.join(EVAL, "full-sut-repair-wave-1-v1", "runs")
...
def gold_binding(case_id, criterion_id, dimension):
    ...
    path = os.path.join(BASE.CORPUS_DIR, os.path.basename(bind["corpus_file"]))
    data = json.load(open(path, encoding="utf-8"))
...
def leakage_audit_extended(packets):
    ...
    for case in old["cases"]:
        for r in case["criteria"]:
            old_verdict[r["criterion_id"]] = r.get("verdict")
    hard = {"EXPECTED_VERDICT_VISIBLE": 0,
            "PRIOR_BASELINE_RESULT_VISIBLE": 0,
            "PRIOR_ASTRA_RESULT_VISIBLE": 0,
            "EXPECTED_REPAIR_EFFECT_VISIBLE": 0}
    prior_meta = {"PRIOR_BASELINE_RESULT_VISIBLE":
                  ("baseline", "remeasure", "wave-1", "wave1"),
                  "PRIOR_ASTRA_RESULT_VISIBLE": ("astra",),
                  "EXPECTED_REPAIR_EFFECT_VISIBLE":
                  ("repair", "rc-01", "rc-02", "rc-03")}
    per = []
    for p in packets:
        pkt = p["packet"]
        ...
        low = content.lower()
        ov = old_verdict.get(p["criterion_id"])
    ...
    return {"artifact": "semantic-review-leakage-audit",
            ...
            "notes": ["expected-verdict scan compares each packet against "
                      "the old baseline derived verdict for the same "
                      "criterion",
...
def stage_a():
    with open(BASE.PERF_JSON, "w", encoding="utf-8") as f:
        json.dump({"source": "structural-120-replay-v2 replay run; no "
    ...
    if len(pred_sha) != 120:
        raise RuntimeError("expected 120 replay predictions, got %d"
                           % len(pred_sha))
    BASE.stage1()
    old_results = load_json(os.path.join(
        EVAL, "measurement-v3-final-authority-provenance-repair-v1",
        "combined-measurement-results-complete-provenance-corrected.json"))

    routing = load_json("measurement-routing-results.json")
    routing["artifact"] = "measurement-routing"
    routing["prediction_source"] = ("evaluation/full-sut-repair-wave-1-v1/"
    ...
    det["prediction_source"] = ("evaluation/full-sut-repair-wave-1-v1/"
                                "runs/structural-120-replay-v2")
    ...
    batch["prediction_set"] = ("evaluation/full-sut-repair-wave-1-v1/runs/"
    ...
    write_json(ASTRA_CONFIG, {
        ...
        "blindness": "B must not see A; neither pass may see historical "
                     "verdicts, gold outcomes, or prior model results",
    ...
    print(json.dumps({
        ...
        "old_baseline_coverage": old_results["coverage"],
    }, ensure_ascii=False))
