#!/usr/bin/env python3
"""Section 22: combine 508 deterministic + 92 semantic criteria into the
Wave-1 measurement result and aggregate metrics."""
import collections
import hashlib
import json
import os
...
def main():
    ...
    coverage = {"TOTAL_CRITERIA": 0, "DETERMINISTIC": 0, "LLM_REVIEWED": 0,
                ...
                "PENDING_HUMAN_ADJUDICATION": 0, "AUTHORITATIVE_TOTAL": 0}
    for case in det["cases"]:
        ...
        for row in case["criteria"]:
            ...
            if row["authority"] == "DETERMINISTIC":
                ...
                continue
            d = obs[cid]
            if d["authority_class"] == "PENDING_HUMAN_ADJUDICATION":
                coverage["PENDING_HUMAN_ADJUDICATION"] += 1
                crit.append({"case_id": row["case_id"], "criterion_id": cid,
                             "dimension": row["dimension"], "authority": "PENDING_HUMAN_ADJUDICATION",
                             "status": "HUMAN_ADJUDICATION_PENDING", "verdict": None,
                             "evidence": [], "provenance": {
    ...
    pending_case_ids = sorted({c["case_id"] for cse in cases
                               ...
                               if c["status"] == "HUMAN_ADJUDICATION_PENDING"})
    results = {"artifact": "wave1-combined-measurement-results",
               "task_id": TASK_ID,
               ...
               "cases": cases}
    with open(os.path.join(HERE, "wave1-combined-measurement-results.json"), "w",
              encoding="utf-8") as f:
    ...
    for dim in ("critical_condition", "forbidden_claim", "route_correctness",
        ...
        per_dim[dim] = {
            ...
            "pending": sum(1 for r in rows
                           if r["status"] == "HUMAN_ADJUDICATION_PENDING"),
            "verdict_distribution": dict(dist)}
    aggregates = {"artifact": "wave1-aggregate-metrics", "task_id": TASK_ID,
                  "created_utc": results["created_utc"],
                  "baseline_label": "BURNED_DEV_BASELINE_ONLY",
                  "interpretation_boundary":
                      "Wave-1 dual-pass Astra agreement is measurement-system "
                      "evidence for the burned dev baseline only; not "
                      "certification, production readiness, or generalization "
                      "evidence.",
                  ...
                  "authority_mix_honesty":
                      "508 deterministic criteria, 88 LLM-reviewed (dual-pass "
                      "Astra LOW consensus), 0 LLM-adjudicated, 4 pending human "
                      "adjudication, 0 human-reviewed; no criterion in this "
                      "remeasurement was decided by a human.",
                  "review_freeze_manifest_sha256":
                      sha(os.path.join(HERE, "semantic-review-freeze-manifest.json"))}
    with open(os.path.join(HERE, "wave1-aggregate-metrics.json"), "w",
              encoding="utf-8") as f:
