...
def _unknown_service_nouns(text_lower, route):
    ...
    for m in re.finditer(r"[a-z\u00e6\u00f8\u00e5]+", text_lower):
        if not any(st in m.group() for st in SERVICE_NOUN_STEMS):
            continue
        if any(s <= m.start() and m.start() < e for s, e in route):
            continue
...
def _route_rules(text, criterion):
    ...
    unknown = [] if route else _unknown_service_nouns(tl, route)
    neg = None
    if route:
