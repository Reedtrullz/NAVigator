#!/usr/bin/env python3
"""Score frozen V2 screening outputs with the exact V1 scorer semantics.

Reference fields come from the frozen V1 reference corpus; the reference is
the authoritative frozen semantic reference. schema_valid_rate counts rows
that produced a parsed object (OK + INVALID_MODEL_REVIEW), matching V1.
Run only after run_screening_v2.py has frozen all candidate outputs.
"""
import json
import os
...
SCREEN = json.load(open(os.path.join(HERE, "semantic-screen-results.json"),
...
def score_candidate(cand_id, lane):
    cand = SCREEN[cand_id]
...
for cand_id in SCREEN:
    out["candidates"][cand_id] = {
        lane: score_candidate(cand_id, lane)
        for lane in ("forbidden_claim", "critical_condition")
    }
with open(os.path.join(HERE, "screening-scored-v2.json"), "w",
          encoding="utf-8") as f:
