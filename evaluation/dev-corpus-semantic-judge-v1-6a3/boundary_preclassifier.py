#!/usr/bin/env python3
"""V1.6A.3 deterministic boundary pre-classifier (model-free, precision-first,
clause-local route grounding with generic ambiguity-abstention gates)."""
...
DISAGREEMENT_MARKERS = ["uenig"]
HYPOTHETICAL_EXPLICIT_MARKERS = ["hypotetisk"]
...
def _route_rules(text, criterion):
    ...
    # ---- V1.6A.3 generic ambiguity gates (frozen: ambiguity-gate-design.md) ----
    disagreement = _hits(tl, DISAGREEMENT_MARKERS)
    hyp_expl = _hits(tl, HYPOTHETICAL_EXPLICIT_MARKERS)

    # G1 quote guard: quote span plus an out-of-quote commitment-relevant
    # signal after the quote end -> endorsement cannot be adjudicated
    # mechanically. Pre-quote signals do not count: a plain assertion
    # followed by a mere quote mention stays deterministic per burned gold.
    if cands and quote and qspans:
        pre_end = qspans[0][1]
        out_quote_ops = [h for h in (assertion + hedge + retraction + cond
                                     + disagreement + hyp_expl + list(neg_hits))
                         if h[1] >= pre_end]
        if out_quote_ops:
            return _abstain("route_commitment", "QUOTE_SCOPE_AMBIGUITY",
                            rule_id="BC_ROUTE_QUOTE_SCOPE_04")

    # G2 parenthetical guard: every route candidate and every commitment
    # operator inside parentheses -> non-standard commitment scope.
    paren_spans = [(m.start(), m.end()) for m in re.finditer(r"\([^)]*\)", tl)]
    if cands and paren_spans:
        def _in_paren(pos):
            return any(s <= pos < e for s, e in paren_spans)

        _ops = assertion + hedge + retraction + cond + quote + list(neg_hits)
        if (all(_in_paren(c["start"]) for c in cands)
                and any(_in_paren(h[1]) for h in _ops)
                and not any(not _in_paren(h[1]) for h in _ops)):
            return _abstain("route_commitment", "PARENTHETICAL_SCOPE_AMBIGUITY",
                            rule_id="BC_ROUTE_PAREN_SCOPE_05")

    # G3 single-candidate mixed polarity: one candidate whose clauses carry
    # 2+ distinct polarity frames (assertion/hedge/negation/conditional)
    # without a retraction resolution -> abstain. Retraction binding keeps
    # SELF_RETRACTED priority.
    if len(cands) == 1 and not retraction:
        frames = set()
        for ci in range(len(clauses)):
            if any(clause_idx(h[1]) == ci for h in neg_hits):
                frames.add("NEGATED")
            elif any(clause_idx(h[1]) == ci for h in cond):
                frames.add("HYPOTHETICAL_ONLY")
            elif any(clause_idx(h[1]) == ci for h in hedge):
                frames.add("HEDGED_ASSERTION")
            elif any(clause_idx(h[1]) == ci for h in assertion):
                frames.add("ASSERTED")
        if len(frames) >= 2:
            return _abstain("route_commitment", "SINGLE_CANDIDATE_MIXED_POLARITY",
                            rule_id="BC_ROUTE_MIXED_POLARITY_04",
                            conflict=[{"frames": sorted(frames)}])

    # G4 hedge competition: hedge-licensed clauses on different candidates
    # separated by eller/men -> operator-to-target pairing unresolved.
    # Parallel hedges without disjunction stay deterministic per burned gold.
    hedge_clauses = sorted({clause_idx(h[1]) for h in hedge} - {-1})
    if len(hedge_clauses) >= 2:
        hedged_c = {c["clause"] for c in cands} & set(hedge_clauses)
        if len(hedged_c) >= 2:
            for a, b in zip(hedge_clauses, hedge_clauses[1:]):
                seg = tl[clauses[a][1]:clauses[b][0]]
                if re.search(r"\b(eller|men)\b", seg):
                    return _abstain("route_commitment", "HEDGE_COMPETITION",
                                    rule_id="BC_ROUTE_HEDGE_COMPETITION_06",
                                    conflict={"hedge_clauses": hedge_clauses,
                                              "separator": seg.strip()})
    ...
    if route:
        for ni in (m.start() for m in re.finditer("ikke", tl)):
            if any(_same_sentence(tl, rh[1], ni) for rh in route):
                # G5: a negation preceded by a conditional marker in the same
                # clause scopes the antecedent condition, not the route
                # proposition; HYPOTHETICAL_ONLY stands.
                nci = clause_idx(ni)
                if nci != -1 and any(clause_idx(h[1]) == nci and h[1] < ni
                                     for h in cond):
                    continue
                neg = ("ikke", ni, ni + 4)
                break
