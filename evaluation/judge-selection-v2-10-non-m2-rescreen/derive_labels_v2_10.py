#!/usr/bin/env python3
"""V2.10 blind curator label passes (INTRA_ANNOTATOR_REPEATABILITY).
...
def _load_generator_prefix():
    src = (HERE / "gen_fixtures_v2_10.py").read_text(encoding="utf-8")
    cut = src.index("def main():")
    ns = {"__file__": str(HERE / "gen_fixtures_v2_10.py")}
    exec(src[:cut], ns)
    return ns
...
def main():
    ...
    dim_of = {f["id"]: ("forbidden" if f["id"].startswith("V210-FORB")
                        else "route" if f["id"].startswith("V210-ROUTE")
    ...
    prepass = {
        "task_id": TASK_ID,
        "method": "Frozen base _expected_path logic from gen_fixtures_v2_10.py plus Phase A extension prepass (classify_extended via boundary-extension-v1-7-draft.py, path-referenced, never modified); deterministic when either fires.",
