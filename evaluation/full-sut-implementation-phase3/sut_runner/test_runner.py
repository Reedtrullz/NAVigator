"""Phase 3 runner tests: port of Phase 2 test_runner.py.

Verifies the one-shot S1-S11 execution harness over the burned dev
corpora: prediction counts, manifest integrity, stage traces, gold
stripping, and secret hygiene."""
...
class TestRunnerFreeze(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.base = tempfile.mkdtemp(prefix="sut-phase3-test-")
    ...
    def test_safety_freeze(self):
        ...
        self.assertEqual(manifest["artifact"], "sut-predictions-manifest-v1-phase3")
        self.assertEqual(manifest["harness_version"], "sut_runner/3.0-phase3")
