"""V2.15 workflow unit tests. One-shot against frozen dev fixtures.
No model calls."""
import json
import os
import sys
...
from review_lane_semantic_v2_15 import (  # noqa: E402
    LaneError, SemanticReviewLane, report_buckets, FORBIDDEN_PACKET_KEYS)
...
for fx in FIX:
    ...
    if expect.get("packet_error"):
        ...
        check("routed_human_review_required", any(
            p.get("event") == "ROUTED" and p.get("route") == "HUMAN_REVIEW_REQUIRED"
        ...
        if expect.get("routed_automated_authoritative_false"):
            check("routed_automated_authoritative_false", any(
                p.get("event") == "ROUTED" and p.get("automated_authoritative") is False
                for p in rec["provenance"]))
        if expect.get("packet_has_no_forbidden_keys"):
            check("packet_has_no_forbidden_keys",
                  not any(k in rec["packet"] for k in FORBIDDEN_PACKET_KEYS))
    results.append({"fixture_id": fid, "category": fx["category"],
                    "dimension": fx["case"].get("dimension") if isinstance(fx.get("case"), dict) else None,
                    "checks": checks, "errors": errs, "pass": not errs})

n_pass = sum(1 for r in results if r["pass"])
summary = {
    "artifact": "workflow-test-results-v2-15",
    ...
    "automated_semantic_verdicts_before_review": 0,
    "results": results,
}
