#!/usr/bin/env python3
"""Blind residual SOL-A/SOL-B for Wave-1 residual adjudication (gpt-5.6-sol, LOW only).

Reuses the frozen primary contract wording and blind-input structure from
run_residual_adjudication.py / run_astra_review.py; only model, effort, and
lineage paths differ. No gold, no primary results, no prior model output."""
import hashlib
...
WAVE1 = os.path.join(os.path.dirname(HERE), "measurement-v3-remeasure-repair-wave-1")
...
_spec = importlib.util.spec_from_file_location(
    "frozen_astra_review", os.path.join(WAVE1, "run_astra_review.py"))
...
def main():
    inp = json.load(open(os.path.join(HERE, "sol-blind-inputs.json"), encoding="utf-8"))
    pkts = inp["packets"]
    transport = {"artifact": "sol-adjudication-transport-log",
                 "task_id": "NAV-EXPLORE-MEASUREMENT-V3-WAVE1-RESIDUAL-LLM-ADJUDICATION-V1",
    ...
    for p in pkts:
        for fname, done, name in ((fa, done_a, "SOL_A"), (fb, done_b, "SOL_B")):
            ...
            time.sleep(1)
    with open(os.path.join(HERE, "sol-transport-log.json"), "w",
              encoding="utf-8") as f:
