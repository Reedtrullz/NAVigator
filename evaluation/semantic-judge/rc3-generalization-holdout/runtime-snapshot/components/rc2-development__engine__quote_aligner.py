...
def extract_ages(text):
    ...
    out = []
    t = str(text or "")
    for m in re.finditer(r"mellom\s+(\d{1,3})\s+og\s+(\d{1,3})\s*"
                         r"(?:\u00e5r|ars|aar|ar)(?!\w)", t, re.IGNORECASE):
        x, y = int(m.group(1)), int(m.group(2))
        out.append({"op": "range", "interval": [x, y],
                    "text": m.group(0), "start": m.start(), "end": m.end()})
    for m in re.finditer(r"(?:fra|fylle?|fylt)\s+(\d{1,3})\s+til\s+(\d{1,3})\s*"
                         r"(?:\u00e5r|ars|aar|ar)(?!\w)", t, re.IGNORECASE):
        x, y = int(m.group(1)), int(m.group(2))
        out.append({"op": "range", "interval": [x, y],
                    "text": m.group(0), "start": m.start(), "end": m.end()})
    for m in re.finditer(r"(?<![\w\u00e6\u00f8\u00e5.])(\d{1,3})\s*-?\s*"
                         r"(?:\u00e5ring|aaring|aring)(?:er|ene)?\b",
                         t, re.IGNORECASE):
        x = int(m.group(1))
        out.append({"op": "bare", "interval": [x, x],
                    "text": m.group(0), "start": m.start(), "end": m.end()})
    for m in re.finditer(r"under\s+(\d{1,3})\s*(?:\u00e5r|ars|aar|ar)?"
                         r"(?!\s*(?:[Gg]\b|ganger\b|kr\b|kroner\b|"
                         r"prosent\b|%))", t,
                         re.IGNORECASE):
        x = int(m.group(1))
