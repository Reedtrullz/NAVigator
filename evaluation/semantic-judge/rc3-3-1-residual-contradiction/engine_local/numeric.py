...
from .temporal import applicability_mismatch, strip_year_phantoms
...
def frac_premise_unverified(claim_text, evidence_text):
    """True when the claim's value is derived from an explicit fraction
    premise ("halvparten av X") whose operand is absent from the evidence.
    The derivation premise is unverified, so neither the claim value nor
    its conflict with evidence numbers is probative until the premise
    resolves. Generalized TEMPORAL_NUMERIC_APPLICABILITY_V1 companion:
    contradiction authorization requires a verified premise, not just a
    value mismatch on the same qualifier/product row."""
    m = re.search(r"halvparten(?:\s+av)?\s+(?:full\s+sats\s+pa\s+)?(%s)"
                  % NUM, claim_text, re.IGNORECASE)
    if not m:
        return False
    operand = _value(m.group(1))
    eqs = parse_quantities(evidence_text) + derived_quantities(evidence_text)
    return not any(eq["value"] == operand
                   and eq["quantity_type"] in ("MONEY", "COUNT")
                   for eq in eqs)
...
def parse_quantities(text):
    ...
    for pat, unit in [(r"maaneder?|maneder?|mnd", "MONTH"), (r"uker?", "DAY"),
...
def _compare(cq, eq):
    ...
    if cq["comparator"] == "=" and not cq.get("range_bounds"):
        ...
        if ((lo < v < hi) or (v == lo and lo_i)
                or (v == hi and hi_i)):
            return ENTAILS
        if eq["comparator"] in ("<", "<=") and v == hi:
            # A bare equality claim at the boundary of a one-sided
            # evidence bound ("< X" / "<= X") is entailed: the evidence
            # region only excludes values beyond X.
            return ENTAILS
        if eq["comparator"] in (">", ">=") and v == lo:
            return ENTAILS
    if _subset(s, c):
        return ENTAILS
    if _disjoint(s, c):
...
def numeric_relation(claim_text, evidence_text):
    ...
    if any(q["quantity_type"] == "MONEY" for q in cqs):
        for q in cqs:
            if q["quantity_type"] == "COUNT" and q["value"] > 500:
                q["quantity_type"] = "MONEY"
                q["unit"] = "NOK"
    # A bare claim-side year ("I 2023 var satsen ...") is a temporal
    # reference, not a countable amount; drop it before comparison.
    cqs = strip_year_phantoms(claim_text, cqs)
    if not cqs:
        return None
    ...
    for i, cq in enumerate(cqs):
        ...
        if cq["value"] % 10000 == 101:
            span = (cq.get("provenance") or {}).get("claim_span") or ""
            if re.fullmatch(r"(1[89]\d{2}|20\d{2})", span.strip()):
                year = int(cq["value"]) // 10000
                pre = claim_text[max(0, claim_text.find(span) - 45):
                                 claim_text.find(span)]
                if (year not in date_years
                        and re.search(r"(?i)(sist\s+endret|endret|endra|"
                                      r"i\s+kraft|fertak)", pre)
                        and re.search(r"\d{4}-\d{2}-\d{2}", evidence_text)):
                    # A bare registry year that matches no evidence stamp
                    # year is a positive identity conflict for
                    # last-changed semantics.
                    date_results.append(CONTRADICTS)
                else:
                    date_results.append(
                        ENTAILS if year in date_years else UNRESOLVED)
            else:
                date_results.append(
                    ENTAILS if cq["value"] in date_full else CONTRADICTS)
        else:
    ...
    if (len(non_date) == 1 and not arithmetic_result
            ...
            and not frac_premise_unverified(claim_text, evidence_text)
            and not any(eq["value"] == non_date[0]["value"]
                        and eq["quantity_type"] == non_date[0]["quantity_type"]
                        for eq in eqs)):
        aggregate_result = CONTRADICTS

    # Gratis carve-out: a claim that a service is free is a positive
    # conflict with any bound positive cost for the same service.
    if any(q["quantity_type"] == "MONEY" and q["value"] == 0.0
           and q["comparator"] == "=" and q.get("object_binding")
           for q in cqs):
        gq = next(q for q in cqs if q["quantity_type"] == "MONEY"
                  and q["value"] == 0.0 and q["comparator"] == "=")
        gtoks = set((gq.get("object_binding") or "").split())
        g_cost = any(
            q["quantity_type"] == "MONEY" and q["value"] > 0
            and q.get("object_binding")
            and gtoks & set(q["object_binding"].split())
            for q in eqs)
        if g_cost:
            return CONTRADICTS
    ...
    # Temporal applicability gate (TEMPORAL_APPLICABILITY_V1): when the
    # claim's temporal frame does not match the deciding evidence, a
    # numeric agreement is stale and cannot ground entailment. A genuine
    # contradiction survives (the mismatch does not erase a conflict).
    mismatch_reason = applicability_mismatch(claim_text, evidence_text)
    if mismatch_reason == "FRAME_ONLY":
        # Pure frame staleness: the evidence never describes the claim's
        # era, so no value agreement or disagreement is probative.
        results = [UNRESOLVED for r in results]
    elif mismatch_reason == "VALUE_STALE":
        # The claim value exists only in a historical evidence clause
        # while the claim asserts the current frame: a numeric agreement
        # is stale, but a real disagreement stays a conflict.
        results = [UNRESOLVED if r == ENTAILS else r for r in results]
