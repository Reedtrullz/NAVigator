#!/usr/bin/env python3
"""Score frozen Luna screening outputs with exact V1 scorer semantics.

Reference fields come from the frozen V1 reference corpus; the reference is
the authoritative frozen semantic reference (not human ground truth).
schema_valid_rate counts rows that produced a parsed object
(OK + INVALID_MODEL_REVIEW + INVALID_JSON-after-fence-fix attempts are not
parsed; only parsed rows count), matching V1/V2 semantics. Run only after
run_screening_luna.py has frozen all candidate outputs.
"""
import json
import os
...
GATES = json.load(open(os.path.join(V1, "qualification-contract.json"),
...
REF = {}
with open(os.path.join(V1, "reference-corpus.jsonl"), encoding="utf-8") as f:
    for line in f:
        if line.strip():
            r = json.loads(line)
            if "observation" in r:
...
def score_config(cid, lane):
    cfg = SCREEN[cid]
    gates = GATES[lane]["screening"]
    rows = [r for r in cfg["results"] if r["lane"] == lane]
    ...
    return {
        "rows": len(rows),
        "ok": len(ok_rows),
        "transport_errors": sum(1 for r in rows if r["status"] == "TRANSPORT_ERROR"),
        "invalid_json": sum(1 for r in rows if r["status"] == "INVALID_JSON"),
        "invalid_model_reviews": sum(1 for r in rows if r["status"] == "INVALID_MODEL_REVIEW"),
...
out = {"scoring_rule": "frozen V1 contract screening gates; reference = frozen authoritative semantic reference, not human ground truth; schema_valid_rate counts parseable rows (V1 semantics)",
       "configs": {}}
for cid in SCREEN:
    out["configs"][cid] = {
        lane: score_config(cid, lane)
        for lane in ("forbidden_claim", "critical_condition")
    }
with open(os.path.join(HERE, "screening-scorecard.json"), "w",
          encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print(json.dumps({cid: {l: {k: v for k, v in s.items()
                            if not k.endswith("hashes")}
                        for l, s in lanes.items()}
                  for cid, lanes in out["configs"].items()},
