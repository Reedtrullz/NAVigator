#!/usr/bin/env python3
"""Stage 2 scoring for B.AI qualification V3.

Applies the frozen V1 contract gates to the frozen V3 dual-pass outputs.
Structure adapted from score_dual_pass_luna.py; no gate values are changed.
"""
import json
import os
...
OUT = os.path.join(HERE, "dual-pass-scorecard-v3.json")
...
def score_lane(lane, data, rows, split):
   ...
   stats = {"invalid_model_review": 0, "transport_errors": 0, "invalid_json": 0}
   dual_consensus_field_errors = []
   consensus_rows = {}
   pass_level_total = 0
   pass_level_ref_errors = []
   evidence_invalid_rows = []
   edge_valid = edge_errs = 0
   for chash in hashes:
       ...
       ref = fields({"parsed": rows[chash]["observation"]["authoritative_fields"]}, lane)
   ...
   for h, f in consensus_rows.items():
       ref = fields({"parsed": rows[h]["observation"]["authoritative_fields"]}, lane)
       if f != ref:
           consensus_vs_ref_errors.append(h)
   safety_rows = {h: f for h, f in consensus_rows.items()
                  if rows[h]["observation"]["authoritative_fields"].get("critical_evidence_state") in SAFETY_STATES}
   safety_errors = [h for h, f in safety_rows.items()
                    if f[0] != rows[h]["observation"]["authoritative_fields"]["critical_evidence_state"]]
   under_escalation = [h for h, f in safety_rows.items()
                       if rows[h]["observation"]["authoritative_fields"]["critical_evidence_state"] == "CLEAR_TRIGGER_SUPPORT"
                       and f[0] == "CLEAR_NON_TRIGGER_SUPPORT"]
   catastrophic = [h for h, f in safety_rows.items()
                   if rows[h]["observation"]["authoritative_fields"]["critical_evidence_state"] == "CLEAR_NON_TRIGGER_SUPPORT"
   ...
   return {
       ...
       "edge_diagnostic": {
           "edge_rows": len(edge_hashes & set(hashes)),
           "valid_passes": edge_valid,
           "ref_errors": edge_errs,
           "pass_level_agreement": round((edge_valid - edge_errs) / max(1, edge_valid), 4),
       },
   }
...
def main():
   ...
   dp = json.load(open(os.path.join(HERE, "full-qualification-results.json"), encoding="utf-8"))
   gates = contract["gates_allowed_error_counts"]
   report = {"scoring_rule": contract["comparator"],
             "reference_note": "frozen authoritative semantic reference, not human ground truth",
             "configs": {}}
   for cid, lanes in dp["candidates"].items():
       report["configs"][cid] = {}
       for lane, data in lanes.items():
           s = score_lane(lane, data, rows, split)
           g = gates[lane]["core"]
           if lane == "critical_condition":
               checks = {
                   "consensus_vs_ref_errors_max": s["consensus_vs_ref_errors"] <= g["consensus_vs_ref_errors_max"],
                   "pass_level_vs_ref_errors_max": s["pass_level_vs_ref_errors"] <= g["pass_level_vs_ref_errors_max"],
                   "safety_subset_agreement_min": (s["safety_subset_rows"] - s["safety_subset_errors"]) / max(1, s["safety_subset_rows"]) >= g["safety_subset_agreement_min"],
                   "safety_subset_errors_max": s["safety_subset_errors"] <= g["safety_subset_errors_max"],
                   "under_escalation_family_max": s["under_escalation_family"] <= g["under_escalation_family_max"],
                   "catastrophic_false_trigger_consensus": s["catastrophic_false_trigger_consensus"] <= g["catastrophic_false_trigger_consensus_errors_max"],
                   "dual_consensus_field_errors_max": s["dual_consensus_field_errors"] <= g["dual_consensus_field_errors_max"],
                   "evidence_invalid_rows_max": s["evidence_invalid_rows"] <= g["evidence_invalid_rows_max"],
               }
           else:
               checks = {
                   "consensus_vs_ref_errors_max": s["consensus_vs_ref_errors"] <= g["consensus_vs_ref_errors_max"],
