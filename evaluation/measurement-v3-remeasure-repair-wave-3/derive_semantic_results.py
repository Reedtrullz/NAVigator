#!/usr/bin/env python3
"""Section 22: mechanical derivation of the semantic review criteria.
...
def main():
    ...
    raw_a, raw_b, raw_sol_a, raw_sol_b = {}, {}, {}, {}
    for name, acc in (("astra-pass-a.jsonl", raw_a),
                      ("astra-pass-b.jsonl", raw_b),
                      ("sol-pass-a.jsonl", raw_sol_a),
                      ("sol-pass-b.jsonl", raw_sol_b)):
    ...
    for p in consensus["packets"]:
        ...
        derived.append({
            ...
            "observation_source": "ASTRA_WAVE3_DUAL_PASS_CONSENSUS",
    ...
    for p in residual["packets"]:
        ...
        if p["consensus_status"] == "LLM_ADJUDICATION_CONSENSUS":
            ...
            derived.append({
                ...
                "observation_source": "SOL_WAVE3_DUAL_PASS_RESIDUAL_CONSENSUS",
                ...
                "derivation_core_sha256": CORE_SHA,
                "derivation_basis": basis, "status": "SCORED"})
        elif p["consensus_status"] == "PENDING_LLM_ADJUDICATION":
            pending.append({
                "criterion_id": meta[pid]["criterion_id"],
                "case_id": meta[pid]["case_id"],
                "dimension": p["dimension"], "packet_id": pid,
                "packet_sha256": None,
                "authority_class": "PENDING_LLM_ADJUDICATION",
                "observation_source": "RESIDUAL_ADJUDICATION_NO_CONSENSUS",
                "observation_model": "gpt-5.6-sol", "reasoning_effort": "low",
                "pending_reason": p.get("reason"),
                "derived_verdict": None,
                "status": "LLM_ADJUDICATION_PENDING"})
    expected = len(packets)
    assert len(derived) + len(pending) == expected, (
        len(derived), len(pending), expected)
