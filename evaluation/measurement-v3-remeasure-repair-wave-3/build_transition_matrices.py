#!/usr/bin/env python3
"""Sections 24-25 + 36: Wave2->Wave3 (primary), Wave1->Wave3 and
Original->Wave3 (secondary) transition matrices, plus the Wave-3
regression inventory.

Comparison runs strictly AFTER the Wave-3 measurement freeze (section 23).
...
def compare(old_doc, new_doc, old_label, gold_changed_case_ids):
    ...
    out = {
        ...
        "comparison_order": "WAVE3_MEASUREMENT_FROZEN_BEFORE_COMPARISON",
        "old_side": old_label,
        ...
        "pending_note": "old-side pending criteria and new-side pending rows "
                        "are excluded from authoritative-only rates"}
    return out


def main():
    w1_rel = ("measurement-v3-wave1-consensus-comparator-repair-v1/"
              "complete-wave1-measurement-results.json")
    w2_rel = ("measurement-v3-remeasure-repair-wave-2/"
    ...
    w1, w2, w3 = load(w1_rel), load(w2_rel), load_here(
        "wave3-combined-measurement-results.json")

    primary = compare(w2, w3,
                      {"label": "MEASUREMENT_V3_REMEASURE_WAVE_2_RESIDUAL_ADJUDICATION_REQUIRED",
                       "path": w2_rel,
                       "sha256": hashlib.sha256(open(os.path.join(
                           EVAL, w2_rel), "rb").read()).hexdigest(),
                       "is_primary": True},
                      set())
    primary["artifact"] = "wave2-wave3-transition-matrix"
    secondary = compare(w1, w3,
                        {"label": "MEASUREMENT_V3_REMEASURE_WAVE_1_COMPLETE",
                         "path": w1_rel,
                         "sha256": hashlib.sha256(open(os.path.join(
                             EVAL, w1_rel), "rb").read()).hexdigest(),
                         "is_primary": False},
                        set())
    secondary["artifact"] = "wave1-wave3-transition-matrix"
    tertiary = compare(orig, w3,
    ...
    tertiary["artifact"] = "original-wave3-transition-matrix"
    for name, doc in (("wave2-wave3-transition-matrix.json", primary),
                      ("wave1-wave3-transition-matrix.json", secondary),
                      ("original-wave3-transition-matrix.json", tertiary)):
        with open(os.path.join(HERE, name), "w", encoding="utf-8") as f:
            json.dump(doc, f, indent=2, ensure_ascii=False)
            f.write("\n")

    inventory = {
        "artifact": "wave3-regression-inventory", "task_id": TASK_ID,
        "created_utc": primary["created_utc"],
        "criteria_checked": 600,
        "comparison": "PRIMARY Wave2 -> Wave3 (section 36)",
        ...
        "lateral_fail_closed_transitions": {
            ...
            "note": "FAIL->UNRESOLVED lateral moves are listed in "
                    "wave2-wave3-transition-matrix.json lateral_criteria"},
