#!/usr/bin/env python3
"""Section 23: combine deterministic and semantic criteria into the
Wave-3 measurement result and aggregate metrics."""
import collections
import hashlib
import json
import os
...
def main():
    ...
    results = {"artifact": "wave3-combined-measurement-results",
               "task_id": TASK_ID,
               ...
               "cases": cases}
    with open(os.path.join(HERE, "wave3-combined-measurement-results.json"), "w",
              encoding="utf-8") as f:
    ...
    aggregates = {"artifact": "wave3-aggregate-metrics", "task_id": TASK_ID,
                  "created_utc": results["created_utc"],
                  "baseline_label": "BURNED_DEV_BASELINE_ONLY",
                  "interpretation_boundary":
                      "Wave-3 dual-pass Astra consensus plus bounded Sol "
                  ...
                  "authority_mix_honesty":
                      "%d deterministic criteria, %d LLM-reviewed (dual-pass "
                      "Astra LOW consensus), %d LLM-adjudicated (dual-pass "
                      "Sol LOW residual consensus), %d pending LLM "
                      "adjudication, %d human-reviewed; no criterion in "
                      "this remeasurement was decided by a human." % (
                          coverage["DETERMINISTIC"], coverage["LLM_REVIEWED"],
                          coverage["LLM_ADJUDICATED"],
                          coverage["PENDING_LLM_ADJUDICATION"],
                          coverage["HUMAN_REVIEWED"]),
                  "review_freeze_manifest_sha256":
                      sha(os.path.join(HERE, "semantic-review-freeze-manifest.json"))}
    with open(os.path.join(HERE, "wave3-aggregate-metrics.json"), "w",
              encoding="utf-8") as f:
