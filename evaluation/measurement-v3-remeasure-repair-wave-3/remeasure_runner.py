#!/usr/bin/env python3
"""Task-local remeasure runner for NAV-EXPLORE-MEASUREMENT-V3-REMEASURE-REPAIR-WAVE-3.

Stage A (default): score the frozen Wave-3 replay predictions with the
FROZEN combined measurement engine, generate fresh semantic review packets
from Wave-3 outputs, run leakage + reviewability audits, and a determinism
re-pass.
...
BASE.P3_RUNS = os.path.join(EVAL, "full-sut-repair-wave-3-v1", "runs")
...
WAVE2_RESULTS = os.path.join(
    EVAL, "measurement-v3-remeasure-repair-wave-2",
...
def prior_verdict_map():
    prior = {}
    for path in (ORIGINAL_BASELINE_RESULTS, WAVE1_RESULTS, WAVE2_RESULTS):
        data = json.load(open(path, encoding="utf-8"))
...
def leakage_audit_extended(packets):
    ...
    return {"artifact": "semantic-review-leakage-audit",
            ...
            "notes": ["expected-verdict scan compares each packet against "
                      "prior Wave-1, Wave-2 and original-baseline verdicts "
...
def stage_a():
    with open(BASE.PERF_JSON, "w", encoding="utf-8") as f:
        json.dump({"source": "full-sut-repair-wave-3-v1 "
                             "structural-120-replay-v1 replay run; no "
    ...
    routing["prediction_source"] = ("evaluation/full-sut-repair-wave-3-v1/"
    ...
    det["prediction_source"] = ("evaluation/full-sut-repair-wave-3-v1/"
                                "runs/structural-120-replay-v1")
    ...
    batch["prediction_set"] = ("evaluation/full-sut-repair-wave-3-v1/runs/"
