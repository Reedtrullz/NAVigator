"""Gold-blind structural replay diagnostics (Wave 2, spec section 33).
...
RUNS = os.path.join(LINEAGE, "runs", "structural-120-replay-v2")
DETERMINISM = os.path.join(LINEAGE, "runs", "determinism-check-v2")
...
def is_fragment_like(label):
    t = label.strip()
    return len(t) < 4 or not any(ch.isalnum() for ch in t)
...
def main():
    ...
    for family, case_id, p in preds:
        ...
        if routes:
            ...
            labels_no_prov = []
            for label in routes:
                entries = [v for k, v in re_.items() if k.startswith("R-")]
                if not entries or not any(v.get("provenance_ids") for v in entries):
                    labels_no_prov.append(label)
            if labels_no_prov:
    ...
    out = {
        ...
        "created_utc": os.environ.get("WAVE2_DIAG_UTC"),
        "candidate_version": "v2",
        "runs_dir": "runs/structural-120-replay-v2/",
        "diagnostic_scope": (
        ...
        "contamination_proxy_note": (
            "Wave-1 metric definitions kept: answers with 2+ 'Nasjonal "
            "informasjon' blocks and KB-markdown-like claim text. RC-08 "
            "scope was cross-domain leakage; same-track INFO blocks are "
            "not counted as contamination"),
        ...
        "route_provenance_rule": (
            "a routed case passes if every route label has at least one "
            "route_evidence entry with provenance_ids"),
        ...
        "determinism_method": (
            "independent full rerun into runs/determinism-check-v2/; "
