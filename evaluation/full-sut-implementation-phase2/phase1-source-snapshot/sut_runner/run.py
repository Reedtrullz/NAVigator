"""One-shot SUT execution harness: strip gold, execute once, freeze.

The runner never scores and has no gold access. Scoring happens in a
separate command against frozen predictions (see execution-harness-design.md).
"""
...
from sut_runner.loader import iter_inputs, load_corpus
...
def execute_case(sut_input, mode):
    """Import the product pipeline and execute it exactly once."""
    from sut.pipeline import run_with_trace
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="SUT one-shot execution harness")
    ...
    run_manifest = {
        "artifact": "sut-predictions-manifest-v1",
        ...
        "harness_version": "sut_runner/1.0-phase1",
    ...
    print(
        "FULL_SUT phase1 freeze complete: %d predictions, %d errors, mode=%s -> %s"
