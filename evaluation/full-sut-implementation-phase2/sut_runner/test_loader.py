"""Phase 2 loader tests: Phase 1 tests ported + location passthrough."""
...
CORPUS_DIR = os.path.join(REPO, "evaluation", "dev-corpus-v1")
ROUTING_FILE = os.path.join(CORPUS_DIR, "cases", "routing_cases.json")
...
class TestStripGold(unittest.TestCase):
    ...
    def test_location_context_passthrough(self):
        case = json.loads(json.dumps(self.case))
        case["location_context"] = {"municipality": "Drammen"}
        inp = strip_gold(case, executed_at="2026-09-14T08:00:00Z")
        self.assertEqual(inp["location_context"], {"municipality": "Drammen"})

    def test_location_context_absent_gives_empty(self):
        case = json.loads(json.dumps(self.case))
        case.pop("location_context", None)
        inp = strip_gold(case, executed_at="2026-09-14T08:00:00Z")
        self.assertEqual(inp["location_context"], {})
