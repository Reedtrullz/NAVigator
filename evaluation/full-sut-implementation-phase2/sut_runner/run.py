"""Phase 2 one-shot SUT execution harness.

Ported from the Phase 1 runner; the only differences are the pipeline
entry point (sut.phase2.pipeline), the manifest artifact name, and the
...
def execute_case(sut_input, mode):
    """Import the Phase 2 pipeline and execute it exactly once."""
    from sut.phase2.pipeline import run_with_trace
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Phase 2 SUT one-shot execution harness")
    ...
    run_manifest = {
        "artifact": "sut-predictions-manifest-v1-phase2",
        ...
        "harness_version": "sut_runner/2.0-phase2",
    ...
    print(
        "FULL_SUT phase2 freeze complete: %d predictions, %d errors, mode=%s -> %s"
