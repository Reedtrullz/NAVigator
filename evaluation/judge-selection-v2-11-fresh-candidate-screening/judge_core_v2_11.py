#!/usr/bin/env python3
"""V2.10 non-M2 judge core (V2.2 lineage; V2.10 Phase B).
...
Changes vs frozen V2.2: UNCLEAR_PROSE added to uncertainty output behaviors
(schema + prompt wording only) and uncertainty derivation routed through the
frozen uncertainty_derivation_v2_7e module (V2.7E contract UNC-A1/UNC-A2).
max_tokens = 32768 (frozen preregistered headroom). No CoT requirement.
V2.10: one preregistered prompt iteration - symmetric PARTIAL-discipline
rule 8; all other prompt content unchanged from frozen V2.9.
"""
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "judge-contract-v2-7e-uncertainty-repair"))
...
6. UNRESOLVED-TVANGSPRINSIPP: Foelgende signaler i kandidatsvaret er foerstedels signaler som krever UNRESOLVED der kontrakten sier det - de skal aldri overses til fordel for en definitt klassifikasjon: (a) selv-heving - samme proposisjon foerst fremsett, saa eksplisitt trukket tilbake uten klar sluttstilling; (b) motstridende begrensning - svaret begrenser seg og overstyrer begrensningen i samme uttrekk; (c) garbled/uklar prosa - limitation-relatert tekst som ikke lar seg klassifisere ved setningsnivaa; (d) commitment som ikke lar seg trygt avgjoere. UNRESOLVED i slike tilfeller er paakrevd korrekt oppfoersel, ikke en daleurdering eller sikkerhetsventil; aa velge definitt verdict her er enklassifiseringsfeil.
7. Ingen kjoerkjede. Kun JSON.
8. PARTIAL-disiplin (symmetrisk): PARTIAL skal kun brukes naar det finnes en
begrunnet funn av delvis oppfyllelse av kriteriet. PARTIAL er aldri en
hedge-fallback for usikkerhet, og en direkte forpliktet assert skal aldri
nedgraderes til PARTIAL bare fordi den er hedget eller delvis formulert.
   Generisk usikkerhet ("jeg er ikke sikker", "sannsynligvis") oppfyller ikke
alene et eksplisitt begrensningskrav i kriteriet: det kreves en konkret
begrensning av samme innhold. En hedget men klar positiv commit er fortsatt
evaluable og scores mot kriteriet som enhver annen commit."""
...
- speaker_commitment (hvis MATCH):
  ...
  * NEGATED: paastanden eksplisitt negert eller fratakket. Vurder commitment
    til den positive proposisjonen (paraphrase av kriteriets innhold), ikke
    til kriteriets negative ordlyd: et svar som bekrefter det positive
    innholdet er ASSERTED/HEDGED_ASSERTION; et svar som avkrefter det er
    NEGATED.
...
- route_speaker_commitment (hvis proposisjon finnes):
  ...
  * SELF_RETRACTED: samme rute foerst positivt fremsett, saa eksplisitt trukket tilbake uten klar sluttstilling.
    Eksplisitt korrigering med klar sluttstilling ("Nei, det stemmer ikke
    lenger", "Beklager, det var feil; X gjelder") avslutter retraksjonen:
    ruten er da ikke lenger committed. Klar sluttstilling er ikke
    SELF_RETRACTED; uten ny konkret rute er commitment UNRESOLVED.
  * UNRESOLVED: commitment ikke trygt avgjorlig.
Steg 2 - proposed_verdict: bare ASSERTED og HEDGED_ASSERTION er evaluable og scores mot kriteriet: ACCEPTABLE (ruten oppfyller kriteriet; semantisk ekvivalent teller), PARTIAL (ruten dekker bare deler av kriteriet), NO_ACCEPTABLE_ROUTE (committed rute feiler kriteriet), UNRESOLVED (ikke trygt avgjorlig). ALLE andre commitment-tilstander skal ha proposed_verdict UNRESOLVED. Hedging sletter ikke ruteidentitet og gjor ikke en ellers korrekt rute til PARTIAL; en hedget positiv rute scores som enhver annen evaluable rute. Hvis kriteriet oppgir flere akseptable alternativer, teller treff paa NOE av alternativene som fullt treff (ACCEPTABLE), ikke PARTIAL. PARTIAL forbeholdes ruter som dekker en identifiserbar del av kriteriet men mangler en noedvendig del (f.eks. tjeneste nevnt men et vilkaar mangler); NO_ACCEPTABLE_ROUTE forbeholdes ruter som peker paa feil sted/tjeneste eller ellers feiler kriteriet i sin helhet. En negert/hypotetisk/sitert rute scores aldri NO_ACCEPTABLE_ROUTE.""",
...
{"uncertainty_requirement_mode":"NONE"|"EXPLICIT_LIMITATION"|"NON_ASSERTION_CONSTRAINT","uncertainty_output_behavior":"NONE"|"HEDGE"|"EXPLICIT_LIMITATION"|"PARTIAL_LIMITATION"|"CONTRADICTORY_LIMITATION"|"UNCLEAR_PROSE"|"OVERCONFIDENT_ASSERTION"|"UNRESOLVED","evidence_basis":"TEXT_SPAN"|"ABSENCE_OF_PROHIBITED_ASSERTION","prohibited_conclusion_checked":"...","evidence_spans":["..."],"note":"kort_snake_case"}
...
NONE (ingen epistemisk signal), HEDGE (vague partikkel alene: "kanskje", "muligens"), EXPLICIT_LIMITATION (svaret uttrykker eksplisitt en begrensning: "er ikke verifisert", "er ikke oppgitt her", "vi har ikke sjekket"), PARTIAL_LIMITATION (identifiserbar men ufullstendig begrensning; inkoherent eller fragmentarisk begrensningstekst er UNCLEAR_PROSE, aldri PARTIAL_LIMITATION), CONTRADICTORY_LIMITATION (motstridende begrensning), OVERCONFIDENT_ASSERTION (fremsetter eller impliserer relevant claim med storre sikkerhet enn kriteriet tillater, ogsaa implisitt: "trolig mangler X", "det kan se ut som X"), UNRESOLVED (kun naar det ikke lar seg avgjore om svaret fremsetter claimet eller dets retning).
For NON_ASSERTION-mode: svaret fremsetter den forbudte konklusjonen hvis den staar som paastand ELLER impliseres => OVERCONFIDENT_ASSERTION. Hvis svaret bare uttrykker usikkerhet om noe annet og hverken fremsetter eller impliserer den forbudte konklusjonen => HEDGE eller EXPLICIT_LIMITATION. Hvis det er reelt uklart om svaret fremsetter konklusjonen => UNRESOLVED. Fravaer av den forbudte konklusjonen er ikke automatisk en markor: hvis svaret har ingen epistemisk signal og heller ikke fremsetter konklusjonen => NONE. UNCLEAR_PROSE: limitation-relatert tekst som ikke lar seg klassifisere som en av tilstandene over (inkoherent eller fragmentarisk ved setningsnivaa) => UNCLEAR_PROSE, som gir UNRESOLVED i avledingen.""",
}
...
def derive_final(dimension, intermediate):
    ...
    if dimension == "required_uncertainty":
        ...
        if mode == "EXPLICIT_LIMITATION":
            return U7E.derive("EXPLICIT_LIMITATION", intermediate["uncertainty_output_behavior"]), "unc:expl"
        if mode == "NON_ASSERTION_CONSTRAINT":
            return U7E.derive("NON_ASSERTION_CONSTRAINT", intermediate["uncertainty_output_behavior"]), "unc:nonassert"
        if mode == "COMPOUND":
            return U7E.derive_compound([(c["kind"], c["behavior"])
                                        for c in intermediate["compound_components"]]), "unc:compound"
    ...
    if dimension == "critical_condition":
        ...
        if mode == "COMPOUND":
            ...
            if inter["uncertainty_output_behavior"] not in BEHAVIORS:
                # unknown/free-text behavior never crashes a screening row;
                # normalized to UNRESOLVED and flagged for the scorer.
                inter["uncertainty_output_behavior"] = "UNRESOLVED"
                inter["_behavior_normalized"] = True
        verdict, basis = derive_final(dimension, inter)
